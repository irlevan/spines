  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatTile label="Books read" value={booksReadThisYear} />
        <StatTile label="Pages read" value={pagesReadThisYear} />
        <StatTile label="Avg rating" value={averageRating !== null ? averageRating.toFixed(1) : "—"} />
          ...
          <div className="card flex flex-col gap-3 p-5">
            {moodEntries.map(([tag, count]) => (
              <div key={tag} className="flex items-center gap-3 text-sm">
                ...
                <span className="font-data w-5 shrink-0 text-right text-subtle">{count}</span>
              </div>
            ))}
          </div>
        ) : (
